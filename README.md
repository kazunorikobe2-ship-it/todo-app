...
| 添付ファイル1つあたりの上限 | 5MB(全プラン共通の固定値) | 5MB(全プラン共通の固定値) | 5MB(全プラン共通の固定値) |
| 添付ファイルの合計容量上限(オーナーの全プロジェクト合算) | 5MB | 150MB | 300MB |
| 共同編集者の人数 | 制限なし | 制限なし | 制限なし |
| プロジェクトの公開共有リンク | 不可 | 不可 | 可 |

1ファイルあたりのサイズ上限はプランに関わらず常に5MBで固定。プランで変わるのは「そのオーナーが持つ全プロジェクトを合算した添付ファイルの合計容量」。合計容量が上限に達すると新しい添付はアップロードできなくなり(既存ファイルの自動削除はしない)、残り容量が10MBを切るとヘッダーに警告バナーが表示される。
...
Storage → Rules タブで以下を貼って公開する。ログイン済みユーザーのみ読み書き可能で、1ファイルあたり5MBまで(プランに関わらず固定、`kazunorikobe2@gmail.com` は常に無制限)をルール側でも強制している:
...
補足: アプリ側(クライアント)でも1ファイルあたりの上限と、プランごとの「合計容量」上限(オーナーの全プロジェクト合算)を事前にチェックしているので、通常はここまで到達する前にアプリ側のメッセージで弾かれる。このルールはファイル単体サイズをすり抜けようとした場合の安全網。合計容量の上限はクライアント側(ブラウザ)でのみチェックしており、Storageセキュリティルール側では強制していない(Firestore上の全プロジェクトの添付ファイルサイズ合計を都度サーバー側で集計する仕組みが必要になるため、現時点では未実装)。
...
プラン(Free/Pro/Business)は実際にプロジェクト数・表示形式・添付ファイル容量・公開共有の可否を制御している(詳細は上の「プラン制限」を参照)。Pro(¥500/月・税別)・Business(¥800/月・税別)はStripeの実際のサブスクリプション課金と連動しており、Freeへのダウングレード・解約は現在の請求期間終了時に反映される。
